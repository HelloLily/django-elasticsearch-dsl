...
class DocTypeMeta(DSLDocTypeMeta):
    def __new__(cls, name, bases, attrs):
        ...
        for field_name, field in iteritems(attrs):
            ...
            for related_model, related_method in iteritems(related):
                related_models[related_model] = 'get_instances_from_{}'.format(
                    related_method
                )
    ...
    def _get_relateds_for_field(cls, name, field):
        ...
        if hasattr(field, '_get_property_fields'):
            for prop_name, prop_field in iteritems(field._get_property_fields()):
                property_related = cls._get_relateds_for_field(
                    prop_name, prop_field)

                for model, method in iteritems(property_related):
                    if model not in related.keys():
                        related[model] = '{}_{}'.format(
                            name, method
                        )
